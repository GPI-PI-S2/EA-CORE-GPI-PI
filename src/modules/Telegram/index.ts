...
export class Telegram extends Extractor {
	private static parsePeer(peer: Telegram.Obtain.Options): Api.GetHistory.Options["peer"] {
		switch (peer.type) {
			...
			case "user":
				return { _: "inputPeerUser", access_hash: peer.accessHash, user_id: peer.chatId } as Api.peerUser;
			default:
	...
	private static parseChats(dialogs: Api.GetDialogs.Response): Telegram.Deploy.chat[] {
		...
		if (Array.isArray(dialogs.chats))
			dialogs.chats.forEach((chat) => (chat._ === "channel" ? channels.push(chat) : chats.push(chat)));
		...
		const readableUsers: Telegram.Deploy.chat[] = users
			...
			.map((user) => {
				let name = "Sin nombre";
				if (user.first_name && user.last_name) name = `${user.first_name} ${user.last_name}`;
		...
		const readableChannels: Telegram.Deploy.chat[] = channels
			.filter((channel) => channel.id && channel.access_hash && !channel.left && !channel.restricted)
			.map((channel) => {
				return { id: channel.id, accessHash: channel.access_hash, type: "channel", name: channel.title };
			});
		extractorChats = extractorChats.concat(readableChannels);

		const readableChats: Telegram.Deploy.chat[] = chats
			.filter((chat) => chat.id && chat.access_hash && !chat.left && !chat.kicked && !chat.deactivated)
			.map((chat) => {
				return { id: chat.id, accessHash: chat.access_hash, type: "group", name: chat.title };
			});
	...
	async deploy(config: Telegram.Deploy.Config, options: Telegram.Deploy.Options): Promise<Response<unknown>> {
		try {
			...
			const chatLimit = (options.chatsLimit && options.chatsLimit) > 0 ? options.chatsLimit : 20;
	...
	async unitaryObtain(options: Telegram.UnitaryObtain.Options): Promise<Response<unknown>> {
		return new Response(this, Response.Status.OK);
	}
	async destroy(options: Telegram.Destroy.Options): Promise<Response<unknown>> {
		return new Response(this, Response.Status.OK);
	}
}
